...
def build_report_css():
    return """
        :root {
            ...
            --card: rgba(20, 22, 30, 0.65);
            --card-border: rgba(255,255,255,0.08);
            --text: #e9ecf5;
            --muted: #9aa3b2;
            --primary: #8aa8ff;
            --primary-600: #6a8cff;
            --accent: #e6c06b; /* premium gold */
            --success: #30d69a;
            --danger: #ff6b88;
            --ring: rgba(230,192,107,0.35);
            --shadow: 0 12px 40px rgba(0,0,0,0.45);
            --glass: saturate(1.2) blur(10px);
        }
        [data-theme="light"] {
            --bg: #f5f6fb;
            ...
            --primary: #5c7dff;
            --primary-600: #4b6cff;
            --accent: #b0892f; /* warm gold */
            --success: #17b26a;
            --danger: #e54861;
            --ring: rgba(92,125,255,0.25);
            --shadow: 0 14px 34px rgba(20,24,40,0.08);
            --glass: saturate(1.1) blur(8px);
        }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        html, body { height: 100%; }
        body {
            font-family: ui-sans-serif, 'Segoe UI', 'Malgun Gothic', Pretendard, Roboto, Arial, sans-serif;
            background:
                radial-gradient(1000px 700px at 100% -10%, rgba(118,92,255,0.22), transparent 60%),
                radial-gradient(1000px 700px at -10% 110%, rgba(230,192,107,0.20), transparent 60%),
                var(--bg);
            color: var(--text);
        ...
        .container::before {
            ...
            background: radial-gradient(800px 500px at 10% -10%, rgba(90, 200, 250, 0.08), transparent 55%),
                        radial-gradient(900px 560px at 110% 110%, rgba(255, 220, 155, 0.08), transparent 50%);
            pointer-events: none;
        }
        .header {
        ...
        .header::after {
            ...
            background: radial-gradient(300px 120px at 85% 0%, rgba(230,192,107,0.12), transparent 60%),
                        radial-gradient(280px 120px at 10% 100%, rgba(138,168,255,0.12), transparent 60%);
        }
        .header > * { position: relative; z-index: 1; }
        .header h1 { font-size: 30px; letter-spacing: -0.3px; margin-bottom: 8px; font-weight: 900; }
        ...
        .stat-item.highlight {
            background: linear-gradient(180deg, rgba(230,192,107,0.16), rgba(230,192,107,0.06));
            border-color: rgba(230,192,107,0.45);
            color: var(--accent);
        }
        .products-grid {
            display: grid;
        ...
        .product-card {
            ...
            cursor: pointer; position: relative; border-width: 2px; border-style: solid; border-color: transparent;
            backdrop-filter: var(--glass);
        }
        .product-card::before {
            content: ""; position: absolute; inset: 0; pointer-events: none;
            background: radial-gradient(300px 120px at 90% 10%, rgba(138,168,255,0.08), transparent 60%);
        }
        .product-card:hover { transform: translateY(-3px); box-shadow: 0 16px 38px rgba(0,0,0,0.22); }
        .product-card.selected { border-color: var(--accent); box-shadow: 0 18px 40px rgba(230,192,107,0.32); transform: translateY(-2px); }
        .product-card.selected::after { content: '✓'; position: absolute; top: 10px; right: 10px; background: var(--accent); color: #0e1118; width: 28px; height: 28px; border-radius: 999px; display: flex; align-items: center; justify-content: center; font-weight: 900; z-index: 10; }
        .product-image { width: 100%; height: 200px; object-fit: contain; background: rgba(0,0,0,0.04); padding: 10px; }
        .product-info { padding: 14px; }
        .product-mall { color: var(--primary); font-size: 12px; margin-bottom: 6px; font-weight: 900; letter-spacing: .2px; }
        .product-title { font-size: 14px; font-weight: 900; color: var(--text); margin-bottom: 10px; line-height: 1.35; display: block; white-space: normal; word-break: break-word; overflow-wrap: anywhere; min-height: 0; }
        .product-price { font-size: 18px; font-weight: 900; color: var(--danger); margin-bottom: 6px; }
        .product-delivery { font-size: 12px; color: var(--success); font-weight: 900; }
        ...
        /* 고정 액션 버튼 */
        .floating-btn { position: fixed; bottom: 26px; right: 26px; background: linear-gradient(135deg, var(--primary-600), var(--primary)); color: white; padding: 12px 18px; border-radius: 999px; font-weight: 900; cursor: pointer; box-shadow: 0 10px 30px rgba(79,124,255,0.35); text-decoration: none; z-index: 1000; border: 0; transition: transform .15s ease, box-shadow .2s ease; }
        .floating-btn:hover { transform: translateY(-2px) scale(1.02); box-shadow: 0 16px 40px rgba(79,124,255,0.45); }
        .highlight-toggle-btn { position: fixed; bottom: 82px; right: 26px; background: linear-gradient(135deg, #f7d37b, var(--accent)); color: #101828; padding: 12px 18px; border-radius: 999px; font-weight: 900; cursor: pointer; box-shadow: 0 10px 30px rgba(230,192,107,0.35); border: none; z-index: 1000; transition: transform .15s ease, opacity .2s ease, box-shadow .2s ease; }
        .highlight-toggle-btn:hover { transform: translateY(-2px) scale(1.02); box-shadow: 0 16px 40px rgba(230,192,107,0.45); }
        .highlight-toggle-btn.off { opacity: .85; filter: saturate(.6); }
        .csv-export-btn { position: fixed; bottom: 138px; right: 26px; background: linear-gradient(135deg, #26dfaa, var(--success)); color: #071911; padding: 12px 18px; border-radius: 999px; font-weight: 900; cursor: pointer; box-shadow: 0 10px 30px rgba(23,178,106,0.35); border: none; z-index: 1000; transition: transform .15s ease, box-shadow .2s ease; }
        .csv-export-btn:hover { transform: translateY(-2px) scale(1.02); box-shadow: 0 16px 40px rgba(23,178,106,0.45); }
        .theme-toggle-btn { position: fixed; bottom: 26px; left: 26px; background: var(--card); border: 1px solid var(--card-border); color: var(--text); padding: 10px 14px; border-radius: 999px; font-weight: 900; cursor: pointer; box-shadow: 0 10px 30px rgba(0,0,0,0.12); z-index: 1000; }
        .delete-info { position: fixed; top: 20px; right: 20px; background: rgba(230,192,107,0.15); color: var(--text); padding: 12px 14px; border-radius: 12px; border: 1px solid var(--card-border); font-weight: 900; z-index: 1000; display: none; backdrop-filter: blur(6px); }
        /* 필터 컨트롤 - 세그먼티드 스타일 */
        ...
        .filter-btn.selected { background: rgba(230,192,107,0.16); border-color: var(--ring); color: var(--text); box-shadow: 0 8px 18px rgba(230,192,107,0.18); }
        /* 하이라이트 스타일 - 프리미엄 언더라인 */
        .highlight { background: none; color: inherit !important; font-weight: 900 !important; box-shadow: inset 0 -0.35em 0 rgba(230,192,107,0.45); }
        .no-highlight .highlight { background: none !important; box-shadow: none !important; font-weight: inherit !important; }
        /* 매칭 카드 강조 */
        .product-card.matched { border-color: rgba(230,192,107,0.85); box-shadow: 0 14px 32px rgba(230,192,107,0.22); }
...
async def create_ready_button(page):
    ...
    return await page.evaluate("""
        (() => {
            try {
                const existing = document.querySelectorAll('#crawling-ready-btn, #crawling-notice');
                existing.forEach(el => el.remove());

                const btn = document.createElement('button');
                btn.id = 'crawling-ready-btn';
                btn.innerHTML = '🚀 크롤링 시작 준비됨';
                btn.style.cssText = `
                    position: fixed !important; top: 18px !important; left: 50% !important; transform: translateX(-50%) !important;
                    z-index: 2147483647 !important; display: inline-flex !important; align-items: center !important; gap: 10px !important;
                    padding: 16px 28px !important; min-width: 320px !important; text-align: center !important;
                    color: #0e1118 !important; font-weight: 900 !important; font-size: 18px !important; letter-spacing: .2px !important;
                    background: linear-gradient(135deg, #f7d37b, #e6c06b) !important; border-radius: 999px !important; border: 0 !important;
                    box-shadow: 0 16px 40px rgba(230,192,107,0.35) !important; cursor: pointer !important;
                    backdrop-filter: saturate(1.1) blur(6px) !important;
                `;

                btn.onmouseenter = () => { btn.style.boxShadow = '0 22px 56px rgba(230,192,107,0.45)'; };
                btn.onmouseleave = () => { btn.style.boxShadow = '0 16px 40px rgba(230,192,107,0.35)'; };
                ...
                btn.onclick = function(e) {
                    ...
                    this.disabled = true;
                    this.style.background = 'linear-gradient(135deg, #48d597, #22b573)';
            ...
            (text) => {
                try {
                    ...
                    container.style.cssText = `
                        ...
                        z-index: 2147483647; width: 460px; padding: 14px 16px;
                        background: rgba(18,20,28,0.85); border-radius: 16px;
                        border: 1px solid rgba(230,192,107,0.45); box-shadow: 0 16px 40px rgba(230,192,107,0.25);
                    ...
                    label.style.cssText = `
                        font-weight: 900; color: #e6c06b; margin-bottom: 8px; text-align: center; letter-spacing: .2px;
                    `;

                    const track = document.createElement('div');
                    track.style.cssText = `
                        width: 100%; height: 12px; background: rgba(255,255,255,0.08); border-radius: 10px; overflow: hidden;
                    `;

                    const bar = document.createElement('div');
                    bar.id = 'crawling-progress-bar';
                    bar.style.cssText = `
                        width: 0%; height: 100%; background: linear-gradient(90deg, #f7d37b, #e6c06b);
        ...
        await page.evaluate(
            ...
            (ok) => {
                ...
                if (bar) {
                    bar.style.width = '100%';
                    if (ok) {
                        bar.style.background = 'linear-gradient(90deg, #48d597, #22b573)';
                    }
                }
                if (label) {
                    label.textContent = ok ? 'HTML 구성 완료' : 'HTML 구성 실패';
                    label.style.color = ok ? '#22b573' : '#e54861';
                }
