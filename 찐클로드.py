...
async def save_products_data(products_data, highlight_keywords=None, page=None):
    ...
    # 공용 CSS: 스크립트와 같은 폴더에 고정 저장
    script_dir = os.path.dirname(os.path.abspath(__file__))
    css_abs_path = os.path.join(script_dir, "naver_shopping_report.css")
    # 결과물 저장 폴더 준비
    ...
    try:
        ...
        # HTML이 저장될 폴더(스크립트 폴더) 기준으로 CSS 상대 경로 계산
        # 브라우저 href 호환을 위해 경로 구분자는 '/' 사용
        css_href_rel = os.path.relpath(css_abs_path, start=results_dir).replace(os.sep, '/')
        ...
        # 공용 CSS가 없으면 한 번만 생성, 있으면 사용자가 수정한 내용 유지
        try:
            if not os.path.exists(css_abs_path):
                with open(css_abs_path, 'w', encoding='utf-8') as cssfile:
                    cssfile.write(build_report_css())
                print(f"CSS 생성 완료(최초 1회): {css_abs_path}")
            else:
                print(f"CSS 재사용: {css_abs_path}")
        except Exception as e:
            print(f"CSS 처리 오류: {e}")
...
<head>
    ...
    {f'<link rel="stylesheet" href="{css_filename}">' if css_filename else ''}
</head>
<body>
    <div class=\"container\">
        <div class=\"header\">
